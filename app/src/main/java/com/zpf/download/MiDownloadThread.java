...
import static com.zpf.download.Downloads.Impl.COLUMN_CONTROL;
import static com.zpf.download.Downloads.Impl.COLUMN_DELETED;
import static com.zpf.download.Downloads.Impl.COLUMN_STATUS;
import static com.zpf.download.Downloads.Impl.CONTROL_PAUSED;
import static com.zpf.download.Downloads.Impl.STATUS_BAD_REQUEST;
import static com.zpf.download.Downloads.Impl.STATUS_CANCELED;
...
import static com.zpf.download.Downloads.Impl.STATUS_TOO_MANY_REDIRECTS;
import static com.zpf.download.Downloads.Impl.STATUS_UNHANDLED_HTTP_CODE;
import static com.zpf.download.Downloads.Impl.STATUS_UNKNOWN_ERROR;
import static com.zpf.download.Downloads.Impl.STATUS_WAITING_FOR_NETWORK;
import static com.zpf.download.Downloads.Impl.STATUS_WAITING_TO_RETRY;
 ...
 */
public class MiDownloadThread extends Thread implements IShutdown {
    ...
    private class DownloadInfoDelta {
        ...
        private ContentValues buildContentValues() {
            ...
            values.put(Constants.SERVER_IP, mServerIp);
...
//        }
    ...
    private boolean tuncateFileToRetry = false;
    ...
    private int orignalExceptionStatus = 0;

    public MiDownloadThread(DownloadJobService service, JobParameters params, DownloadInfo info) {
        mContext = service;
...
//        }
        ...
        if (mShutdownRequested) {
            logDebug("Already Shutdown; skipping");
            return;
        }
...
//            TrafficStats.incrementOperationCount(1);
            ...
            orignalExceptionStatus = mInfoDelta.mStatus;
            mInfoDelta.mErrorMsg = e.getMessage();

            logWarning("Stop requested with status "
                    + Downloads.Impl.statusToString(mInfoDelta.mStatus) + ": "
                    + (mInfoDelta.mErrorMsg == null ? "null" :
                    new String(Base64.encode(mInfoDelta.mErrorMsg.getBytes(), Base64.DEFAULT))));
...
//            boolean skipWriteDb = false;
//            if (mShutdownRequested &&
//                    mInfoDelta.mStatus == STATUS_WAITING_TO_RETRY) {
//                skipWriteDb = mInfo.queryDownloadControl() != 0;
//            }
//            if (!skipWriteDb) {
//                finalizeDestination();
//                mInfoDelta.writeToDatabase();
...
//            TrafficStats.clearThreadStatsUid();
        ...
        notifyListener((int) mInfo.mId, mInfoDelta.mCurrentBytes, mInfoDelta.mTotalBytes);
...
//        boolean cleartextTrafficPermitted = mSystemFacade.isCleartextTrafficPermitted(mInfo.mUid);
        boolean cleartextTrafficPermitted = mSystemFacade.isCleartextTrafficPermitted(mInfo.mPackage, url.getHost());
        ...
        while (redirectionCount++ < Constants.MAX_REDIRECTS) {
            ...
            try {
                ...
                DownloadInfoManager.INSTANCE.onStartLoad(((int) mInfo.mId));

//                if (Utils.isOTAUpdater(mInfo.mPackage)) {
//                    try {
//                        conn.connect();
//                    } catch (Exception e) {
//                        e.printStackTrace();
//                    }
//                    mInfoDelta.mServerIp = HttpURLConnectionHelper.getServerIp(conn);
//                }

                final int responseCode = conn.getResponseCode();

                //[MIUI add start]
                logInfo("executeDownload responseCode = " + responseCode + ","
                        + new String(Base64.encode(url.toString().getBytes(), Base64.DEFAULT)));
                if (mShutdownRequested) {
                ...
                if (e instanceof ProtocolException
                    ...
                    throw new StopRequestException(STATUS_HTTP_DATA_ERROR,
                            "error occurred connecting to the server.", e);
                }
    ...
    private void transferData(HttpURLConnection conn) throws StopRequestException {
        ...
        try {
            try {
                ...
                throw new StopRequestException(STATUS_HTTP_DATA_ERROR, "error occurs while" +
                        " creating the input stream or the protocol does not support input.", e);
            }
...
//            }
    ...
    private void transferData(InputStream in, OutputStream out, FileDescriptor outFd)
        ...
        while (true) {
            ...
            try {
                ...
                checkFileExists(mInfoDelta);
            } catch (FileNotFoundException e) {
                tuncateFileToRetry = true;
                throw new StopRequestException(STATUS_FILE_ERROR, e);
            } catch (ErrnoException e) {
                throw new StopRequestException(STATUS_FILE_ERROR, e);
            } catch (IOException e) {
    ...
    private void checkFileExists(DownloadInfoDelta downloadInfoDelta) throws FileNotFoundException {
        if (!new File(downloadInfoDelta.mFileName).exists()) {
            throw new FileNotFoundException("file not fount in download process");
        }
    }
    ...
    private void finalizeDestination() {
        if (Downloads.Impl.isStatusError(mInfoDelta.mStatus) || tuncateFileToRetry) {
            //[MIUI add start]
//            if (Utils.isMiPicks(mInfo.mPackage) && isStatusRetryable(mInfoDelta.mStatus)) {
//                logDebug("won't delete localfile ,mInfoDelta.mStatus = " + mInfoDelta.mStatus);
//                return;
//            }
            //[MIUI add end]
...
//            mInfoDelta.writeToDatabaseOrThrow();
            ...
            mLastUpdateTime = now;
            notifyListener(((int) mInfo.mId), currentBytes, mInfoDelta.mTotalBytes);
        }
    }
    ...
    private void parseOkHeaders(HttpURLConnection conn) throws StopRequestException {
        ...
        logInfo("parseOkHeaders. TotalBytes = " + mInfoDelta.mTotalBytes + ",transferEncoding = " + transferEncoding);
...
//        mInfoDelta.writeToDatabaseOrThrow();
    ...
    private void parseUnavailableHeaders(HttpURLConnection conn) {
        ...
        retryAfter = MathUtils.clamp(retryAfter, Constants.MIN_RETRY_AFTER,
    ...
    //[MIUI add start]
    protected void washTaskStart() {
        mStartTime = System.currentTimeMillis();
        mStartSize = mInfo.mCurrentBytes;
    }

    protected void washTaskStop() {
        long dTime = System.currentTimeMillis() - mStartTime;
        long dSize = mInfoDelta.mCurrentBytes - mStartSize;
        DownloadInfo info = new DownloadInfo(mContext);
        info.mPackage = mInfo.mPackage;
        info.mFileName = mInfoDelta.mFileName;
        info.mCurrentBytes = mInfoDelta.mCurrentBytes;
        info.mTotalBytes = mInfoDelta.mTotalBytes;
        info.mUri = mInfoDelta.mUri;
        info.mMimeType = mInfoDelta.mMimeType;
    }
}